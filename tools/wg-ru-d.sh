...
for file_path in "${files[@]}"; do
    if grep -q "$file_search_string" "$file_path"; then
        ...
        sed -i "s|There are no clients yet.|Клиенты ещё добавлены! Создайте!|" "$file_path"
        sed -i "s|>New Client<|>Новый клиент<|" "$file_path"
        ...
        sed -i 's|h1 class="text-4xl dark:text-slate-200 font-medium mt-2 mb-2"|h1 class="text-2xl dark:text-slate-200 font-medium mt-2 mb-2"|g' "$file_path"
